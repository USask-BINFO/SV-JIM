...
#Ensure required parent directories are set up before execution
...
then
	echo "Output Dir already exists! Removing to allow truvari to run!.. "
...
truvari bench -o $OUTPUT_DIR/$SV_TYPE/Call/FromTPCall -f $REF_GENOME -b ${TRUVARI_DIR_A}/tp-call.sorted.vcf.gz -c ${TRUVARI_DIR_B}/tp-call.sorted.vcf.gz -s 0 -S 0 --sizemax $MAX_SIZE -r $MAX_REF_DIST -p 0 -P $OVERLAP -C $MAX_REF_DIST
truvari bench -o $OUTPUT_DIR/$SV_TYPE/Call/FromTPBase -f $REF_GENOME -b ${TRUVARI_DIR_A}/tp-call.sorted.vcf.gz -c ${TRUVARI_DIR_B}/tp-base.sorted.vcf.gz -s 0 -S 0 --sizemax $MAX_SIZE -r $MAX_REF_DIST -p 0 -P $OVERLAP -C $MAX_REF_DIST
truvari bench -o $OUTPUT_DIR/$SV_TYPE/Base/FromTPCall -f $REF_GENOME -b ${TRUVARI_DIR_A}/tp-base.sorted.vcf.gz -c ${TRUVARI_DIR_B}/tp-call.sorted.vcf.gz -s 0 -S 0 --sizemax $MAX_SIZE -r $MAX_REF_DIST -p 0 -P $OVERLAP -C $MAX_REF_DIST
#truvari bench -o $OUTPUT_DIR/$SV_TYPE/Base/FromTPBase -f $REF_GENOME -b ${TRUVARI_DIR_A}/tp-base.sorted.vcf.gz -c ${TRUVARI_DIR_B}/tp-base.sorted.vcf.gz -s 0 -S 0 --sizemax $MAX_SIZE -r $MAX_REF_DIST -p 0 -P $OVERLAP -C $MAX_REF_DIST
