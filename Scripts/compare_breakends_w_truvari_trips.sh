...
#Ensure Parent directories are set up before Truvari execution
mkdir -p $OUTPUT_DIR/$SV_TYPE
...
truvari bench -o $OUTPUT_DIR/$SV_TYPE/FromTPCall -f $REF_GENOME -b ${SV_FILE_A}.gz -c ${TRUVARI_DIR}/tp-call.sorted.vcf.gz -s 0 -S 0 --sizemax $MAX_SIZE -r $MAX_REF_DIST -p 0 -P $OVERLAP -C $MAX_REF_DIST
truvari bench -o $OUTPUT_DIR/$SV_TYPE/FromTPBase -f $REF_GENOME -b ${SV_FILE_A}.gz -c ${TRUVARI_DIR}/tp-base.sorted.vcf.gz -s 0 -S 0 --sizemax $MAX_SIZE -r $MAX_REF_DIST -p 0 -P $OVERLAP -C $MAX_REF_DIST
