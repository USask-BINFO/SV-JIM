...
truvari bench -o $OUTPUT_DIR/Call/FromTPCall -f $REF_GENOME -b ${TRUVARI_DIR_A}/tp-call.sorted.vcf.gz -c ${TRUVARI_DIR_B}/tp-call.sorted.vcf.gz -s $MIN_SIZE --sizemax $MAX_SIZE -r $MAX_REF_DIST -p 0 -P $OVERLAP -C $MAX_REF_DIST
truvari bench -o $OUTPUT_DIR/Call/FromTPBase -f $REF_GENOME -b ${TRUVARI_DIR_A}/tp-call.sorted.vcf.gz -c ${TRUVARI_DIR_B}/tp-base.sorted.vcf.gz -s $MIN_SIZE --sizemax $MAX_SIZE -r $MAX_REF_DIST -p 0 -P $OVERLAP -C $MAX_REF_DIST
truvari bench -o $OUTPUT_DIR/Base/FromTPCall -f $REF_GENOME -b ${TRUVARI_DIR_A}/tp-base.sorted.vcf.gz -c ${TRUVARI_DIR_B}/tp-call.sorted.vcf.gz -s $MIN_SIZE --sizemax $MAX_SIZE -r $MAX_REF_DIST -p 0 -P $OVERLAP -C $MAX_REF_DIST
